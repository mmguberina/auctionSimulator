...
def plotAveragePayoffConfidenceInterval():
    ...
    auctions_per_strategy_update_slice = [10, 20, 50, 100, 150, 200, 250, 333, 500, 1000]
...
if __name__ == "__main__":
    ...
    fil = open("./auctionsPerEpochData_" +experiment_id + ".pkl", 'rb')
    results = pickle.load(fil)
    fil.close()

    fig, ax = plt.subplots()
    labels = auctions_per_strategy_update_slice
    x = np.arange(len(auctions_per_strategy_update_slice))
    rects = []
    width = 0.35
    for i, payment_method in enumerate(results):
        ...
        rects.append(ax.bar(x - width/2 + i * width, np.array(results[payment_method]),
            width=width, label=payment_method))

    ax.set_ylabel('Half of 95% confidence interval')
    ax.set_xlabel('Number of auctions in a slice')
    ax.set_title('95% confidence interval half of average payoff per auction number')
    ax.set_xticks(x, labels)
    ax.legend()

    fig.tight_layout()
    plt.savefig("conf_interval_per_n_of_auctions.png", dpi=600)
